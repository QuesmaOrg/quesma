...
func TimestampGroupBy(timestampField model.Expr, typ DateTimeType, groupByInterval time.Duration) model.Expr {
	...
	createAExp := func(innerFuncName string, interval int64, offsetMultiplier int64) model.Expr {
		toUnixTsFunc := model.NewInfixExpr(
			model.NewFunction(innerFuncName, timestampField),
			" * ",
			model.NewLiteral(offsetMultiplier))
		...
		toUnixTsFunc = model.NewInfixExpr(toUnixTsFunc, " / ", model.NewLiteral(interval))
		return model.NewFunction("FLOOR", toUnixTsFunc)
	}

	switch typ {
	case DateTime64:
		// as string: fmt.Sprintf("toInt64(toUnixTimestamp(`%s`)/%f)", timestampFieldName, groupByInterval.Seconds())
		return createAExp("UNIX_TIMESTAMP", groupByInterval.Milliseconds(), 1000)
	case DateTime:
		return createAExp("UNIX_TIMESTAMP", groupByInterval.Milliseconds()/1000, 1)
	default:
