...
func AnalyzeUnsupportedCalls(ctx context.Context, method, path string, indexResolver func(pattern string) (indexes []string)) (result bool) {
...
func checkSearchTemplateWithIndex(ctx context.Context, method string, path string, indexResolver func(string) []string) bool {
	...
	for _, rx := range searchTemplatePathWithIndexRegexps {
		if rx.MatchString(path) {
			...
			if len(match) > 1 {
				for _, indexName := range indexResolver(match[1]) {
...
func checkIfOurIndex(ctx context.Context, method string, path string, indexResolver func(string) []string) bool {
	...
	if len(match) > 1 {
		...
		var matched bool
		for _, indexName := range indexResolver(indexNamePart) {
			matched = true
