			...
			FROM (
			  ...
			  FROM (
				SELECT sum(count(*)) OVER () AS "aggr__terms__parent_count",
				  ...
				  )) AS "aggr__terms__large-grid__key_0",
				  FLOOR((1-LOG(TAN(RADIANS(toFloat64(__quesma_geo_lat("OriginLocation"))))+(
				  1/COS(RADIANS(toFloat64(__quesma_geo_lat("OriginLocation"))))))/PI())/2*
				  POWER(2, 8)) AS "aggr__terms__large-grid__key_1",
