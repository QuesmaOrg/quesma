...
func Test_parsePercentilesAggregationWithDefaultPercents(t *testing.T) {
	...
	fieldName, _, userSpecifiedPercents := cw.parsePercentilesAggregation(payload)
...
func Test_parsePercentilesAggregationWithUserSpecifiedPercents(t *testing.T) {
	...
	assert.True(t, isBetween(parsedMap["11.123123123123124"].(float64), 0.111231231, 0.111231232))
	assert.Equal(t, 0.634, parsedMap["63.4"])
	assert.True(t, isBetween(parsedMap["66.999999999999"].(float64), 0.66999999, 0.67))
	assert.Equal(t, 0.95, parsedMap["95"])
	assert.Equal(t, 0.99, parsedMap["99"])
	assert.True(t, isBetween(parsedMap["99.9"].(float64), 0.999, 0.9991))
