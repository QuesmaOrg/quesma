...
func Test_OsdHeaders(t *testing.T) {
	...
	assert.Equal(t, "application/json; charset=UTF-8", response.Get(contentTypeHeaderKey), "Content-queryType set correctly")
}
...
func Test_EsHeaders(t *testing.T) {
	...
	assert.Equalf(t, "application/vnd.elasticsearch+json;compatible-with=8", response.Get(contentTypeHeaderKey), "Content-queryType set correctly")
}
