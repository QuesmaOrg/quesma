...
func (qmc *QuesmaManagementConsole) generateLoginForm() []byte {
	...
	buffer.Html(`<head>`)
	buffer.Html(`<style>`)
	buffer.Html(`body { font-family: Arial, sans-serif; background-color: #f4f4f4; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }`)
	buffer.Html(`.login-form { background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); }`)
	buffer.Html(`.login-form h2 { margin-bottom: 20px; }`)
	buffer.Html(`.login-form label { display: block; margin-bottom: 5px; }`)
	buffer.Html(`.login-form input[type="text"], .login-form input[type="password"] { width: 100%; padding: 8px; margin-bottom: 10px; border: 1px solid #ccc; border-radius: 4px; }`)
	buffer.Html(`.login-form input[type="submit"] { width: 100%; padding: 10px; background-color: #007BFF; border: none; border-radius: 4px; color: #fff; font-size: 16px; cursor: pointer; }`)
	buffer.Html(`.login-form input[type="submit"]:hover { background-color: #0056b3; }`)
	buffer.Html(`</style>`)
	buffer.Html(`</head>`)
	buffer.Html(`<body>`)
	buffer.Html(`<div class="login-form">`)
	buffer.Html(`<h2>Login</h2>`)
	buffer.Html(`<p>Log in to Quesma admin console using your Elasticsearch credentials</p>`)
