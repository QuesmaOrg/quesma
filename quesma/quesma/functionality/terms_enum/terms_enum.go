...
func HandleTermsEnum(ctx context.Context, index string, body types.JSON, lm clickhouse.LogManagerIFace,
	...
	if indices, err := lm.ResolveIndexPattern(ctx, schemaRegistry, index); err != nil || len(indices) != 1 { // multi index terms enum is not yet supported
		...
		return handleTermsEnumRequest(ctx, body, lm, &elastic_query_dsl2.ClickhouseQueryTranslator{Table: lm.FindTable(indices[0]), Ctx: context.Background(), Schema: resolvedSchema}, qmc)
	}
}

func handleTermsEnumRequest(ctx context.Context, body types.JSON, lm clickhouse.LogManagerIFace, qt *elastic_query_dsl2.ClickhouseQueryTranslator,
	...
	field = elastic_query_dsl2.ResolveField(ctx, field, qt.Schema)
